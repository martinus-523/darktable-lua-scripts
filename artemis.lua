...
-- Artemis|English|Birds|Perching Birds|Tits and Chickadees|Great Tit. The
-- tagger only descends the taxonomy as far as it is confident, so an unclear
-- photo may end at order or family level. Images already accepted in the
-- nature review panel (any tag under nature's prefix, Nature| by default) are
-- never scanned. Everything runs locally; see readme.md for the one-time uv
-- setup (the first run downloads ~7 GB of model data).

local this_module = ...
local folder = this_module and this_module:match("^(.*[/\\])") or ""

local dt = require "darktable"
-- dt.control.execute() cannot handle a command with more than one quoted
-- item on Windows; dtutils.system.windows_command() runs it via a batch
-- file instead. Ships with darktable, no extra install needed.
...
-- The Windows branch matters: HOME is unset there (it is USERPROFILE) and
-- none of the POSIX paths exist, so this used to fall through to a bare
-- "uv" that only worked if darktable inherited a PATH containing it.
...
-- compare paths regardless of slash direction (Windows gives us
-- C:\Photos\IMG.CR3, Python may hand back either form)
...
local function tag_images(images)
  ...
  -- CHANGED (5): force UTF-8 for all of Python's default file encodings.
  -- tagger.py already declares encoding="utf-8" on every read/write, but
  -- this protects any future open() added there. Works because
  -- windows_command runs the whole string through a batch file.
  if dt.configuration.running_os == "windows" then
  ...
  -- the execute call yields this coroutine while the tagger runs, so a
  ...
  -- windows_command handles multi-quoted commands via a batch file
  local rc
  if dt.configuration.running_os == "windows" then
...
-- CHANGED (7): darktable 5.6 sets the global darktable_gui_safe when it is
-- safe to install a lib; registering earlier silently drops the button
-- because of a startup race that reports lighttable before it is
-- initialized.
...
if darktable_gui_safe then
  ...
  dt.register_event(MODULE .. "_gui", "view-changed",
    function(_, old, new)
      if not registered and new.id == "lighttable" and darktable_gui_safe then
        register_ui()
      end
    end)
end
...
-- CHANGED (8): script_manager expects a table with destroy/restart so the
-- script can be started and stopped from the scripts module.
local script_data = {}
script_data.metadata = {
  name = "artemis",
  purpose = "animal and bird species identification with BioCLIP",
  author = "",
  help = "",
}
