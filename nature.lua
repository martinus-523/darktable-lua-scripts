...
-- CHANGED: tooltips were built once at load from the preference values, so
-- changing the artemis or nature prefix left them stale until restart.
local function refresh_tooltips()
  local a, n = artemis_prefix(), nature_prefix()
  accept_button.tooltip = "copy the " .. a .. " tags as " .. n
    .. " tags (a preference controls whether the " .. a .. " tags are removed)"
  reject_button.tooltip = "remove the " .. a .. " tags from this image"
end

local function update_panel()
  local selection = dt.gui.selection()
  refresh_tooltips()  -- CHANGED
...
local function reject()
  ...
  if not image then update_panel() return end  -- CHANGED: refresh on no-op
  local artemis_tags = reviewable(image)
...
-- CHANGED: darktable 5.6 sets darktable_gui_safe when it is safe to install
-- a lib. Registering during startup can silently fail because the view
-- reports lighttable before it is actually initialized.
...
-- CHANGED: libs cannot be destroyed via the API, so script_manager uses the
-- hide/show pattern for lib-based scripts.
local script_data = {}
script_data.metadata = {
  name = "nature",
  purpose = "review and accept artemis identifications",
  author = "",
  help = "",
}
script_data.destroy = function()
  if dt.gui.libs[MODULE] then dt.gui.libs[MODULE].visible = false end
  dt.destroy_event(MODULE, "selection-changed")
end
script_data.destroy_method = "hide"
script_data.show = function()
  if dt.gui.libs[MODULE] then dt.gui.libs[MODULE].visible = true end
end
